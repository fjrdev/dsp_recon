...
void fbi(int phase_res_u, int phase_res_v, int res_u, int res_v, int fft_im_cols, int num_tx, int num_rx, 
        complexf *fft_im, float *pos_tx, float *o, float *u, float *v, int sig_len, float dx, float dz, 
        float FLT_EPSILON, float c, float t0, float t1, complexf *fft_data, int Z0, float v_len, float u_len) {
    ...
    for(int c_idx_u=0; c_idx_u<phase_res_u;c_idx_u++) {
        for(int c_idx_v=0; c_idx_v<phase_res_v;c_idx_v++) {
            ...
            for(int i_tx=0; i_tx<num_tx; i_tx++) {
                ...
                const float diff_len = sqrt(center_tx_diff[0]*center_tx_diff[0] + center_tx_diff[1]*center_tx_diff[1]);
                ...
                #pragma omp parallel for num_threads(2) shared(tx_position, chunk_center, center_tx_diff, diff_len, n, rfft_cols, rfft_rows, i_tx, c_idx_v, c_idx_u, fft_im, fft_data)
                for (int iv = 0; iv < fft_im_cols; iv++) {
                    for (int iu = 0; iu < res_u / 2; iu++) { // positive frequencies
                        ...
                        bool is_active = (kz > 0) && (g_omega < sig_len / 2) && (k0>0.0);
